...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const dinosaur = PIXI.Sprite.from("./img/Brachiosaurus.png");
      dinosaur.anchor.x = 0.5;
      dinosaur.anchor.y = 0.5;
      initWidth = dinosaur.width;
      dinosaur.scale.set(growth / initWidth);
      ...
      setInterval(() => {
        growth = 50 + ateNum.value;
        dinosaur.scale.set(growth / initWidth);
      }, 1000);

      setInterval(() => {
        if (meatArr.length >= 30) return;
        const meat = PIXI.Sprite.from("./img/plant_buna_leaf.png");
        meat.anchor.x = 0.5;
        meat.anchor.y = 0.5;
